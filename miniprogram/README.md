...
`https://askqiankun.com/api/miniprogram/perfumes/resolve`。识别成功的公开商品资料会
经至少两个独立来源支持且置信度足够高后，进入不关联账号的共享香水库；同一香水
后续命中为 0 token。只有共享库也未命中时，
才检索少量公开资料并用低输出预算模型提取香调。识别结果必须由用户
