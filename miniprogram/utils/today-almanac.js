...
// 今日一色盘:今日五行 + 宜色(所生=贵人色)+ 避色(所克)。各页统一走这里,口径不漂移。
// 「今日五行」口径 = 日支五行,经博主实测两天交叉锁定(非纳音、非日干):
//   7/16 辛卯 日支卯木 → 木生火 → 宜红(博主 CJLA15 原文一致)
//   7/23 戊戌 日支戌土 → 土生金 → 宜白(博主一致;此日纳音=木会误给红,日干=土碰巧同,故用日支)
function dayPalette(ganZhi) {
  const zhi = ganZhi[1];
  const el = ZHI_WX[zhi] || '土';
